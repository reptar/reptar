...
export default class Yarn {
  ...
  constructor(options = {}) {
     ...
     * @type {Config}
     */
    this.config = Config.create(options.rootPath);
    ...
    cache.setNamespace(this.config.root);
    ...
    const shouldLoadCache = !_.isNil(options.incremental) ?
  ...
  /**
   * Reload state of Yarn from file system.
   * @return {Promise}
   */
  async reload() {
    this.config.update();
  ...
  async build() {
    ...
    const fileActivityId = log.startActivity('Writing files.\t\t\t');
    let filePromises;
    try {
      filePromises = _.reduce(this.files, (result, file) => {
    ...
    const collectionActivityId = log.startActivity(
      'Writing collection pages.\t'
    );
    try {
      promise = await Promise.all(_.map(this.collections, collection => {
