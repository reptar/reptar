...
class Whee {
  ...
  loadConfig(configDir) {
    ...
    try {
      ...
      logger.log('warn', 'Unable to load local config.yml file.');
    }
  ...
  readFiles() {
    let files = glob.sync(this.config.absPath.source + '/**/*.md');
    if (!files.length) {
      return;
    }
  ...
  writeFiles() {
    this.files.map(file => {
      ...
      let writePath = path.join(this.config.absPath.destination, safePermalink);
