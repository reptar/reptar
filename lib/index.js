...
export default class Yarn {
  ...
  configUpdated() {
    Url.setSlugOptions(this.config.slug);
    ...
    Render.configureMarkdown(this.config.remarkable);

    /**
     * Expose site data from config file.
     * @type {Object}
     */
    this.data.site = this.config.site;

    // Update our collection configs.
    map(this.config.collections, (collectionConfig, collectionName) => {
  ...
  async readFiles() {
    ...
    // Built-in plugins.
    Plugin.loadFromPackageJson(Url.pathFromRoot('./'), this.config.plugins);
    [
      this.theme.config.path.plugins, // Active theme plug-ins
      this.config.path.plugins // Site plug-ins
    ].forEach(path => Plugin.loadFromDirectory(path));
    logger.info('\tdone!');

    logger.info('Reading files...');
    let ignorePatterns = values(CONSTANTS.YAML).concat(
      values(this.config.path).filter(path => path !== this.config.path.source)
    );
    ...
    let files = await Promise.fromCallback(cb => {
      glob(this.config.path.source + '/**/*', {
        nodir: true,
        ignore: ignorePatterns.map(path => `**/${path}/**`).concat(
          `${this.config.path.source}/node_modules/**`
        ),
        // Follow symlinks.
        follow: true
      }, cb);
  ...
  async cleanDestination() {
    ...
    logger.info('Cleaning destination %s', this.config.path.destination);
    try {
      promise = await Promise.fromCallback(cb => {
        rimraf(this.config.path.destination, cb);
      });
    } catch (e) {
      logger.error(e);
    }
  ...
  async build() {
    ...
    if (this.config.clean_destination) {
