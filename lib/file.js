...
export default class File {
  ...
  updateDataFromFileSystem() {
     ...
     * @type {string}
     */
    this.checksum = createChecksum(this.rawContent);

    // Parse file's frontmatter.
    const parsedFile = Parse.fromFrontMatter(this.rawContent);
    ...
    _.merge(this.data, parsedFile.data, {
  ...
  _calculateDestination() {
    ...
    if (this.url) {
      ...
      const fileExtension = path.extname(destinationUrl).replace(/^\./, '');
  ...
  write(siteData) {
    ...
    for (const collection of this.collections) {
      // Don't render if we have no template defined.
      if (_.isUndefined(collection.template)) {
        continue;
      }

      const promise = renderAndWriteFileWithPlugins(
        this,
        collection.template,
        siteData,
