...
class Collection {
  ...
  constructor(name, collectionConfig = {}) {
     ...
     * @type {boolean}
     */
    this.static = collectionConfig.static || false;
  ...
  async write(siteData) {
    if (this.static) {
      try {
        await fs.copyAsync(this.path, this.staticDestination);
      } catch (e) {
        logger.error(`Collection: Could not copy Collection '${this.path}'.`);
      }
