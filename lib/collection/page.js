...
export default class CollectionPage {
   ...
   */
  setFiles(files) {
    if (!isUndefined(files) && !isArray(files)) {
  ...
  * @param {CollectionPage} next CollectionPage instance.
  ...
  _calculateDestination() {
    ...
    let fileSystemSafeDestination = Url.makeUrlFileSystemSafe(
      relativeDestination
    );
  ...
  render(template = 'default', globalData) {
    ...
    return Render.fromTemplate(template, templateData);
  }
}
