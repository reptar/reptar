...
export default class Theme {
  ...
  update() {
    this.name = this._getConfig().theme;

    // Find all theme config files that might exist.
    const fileSystemYamls = glob.sync(
      this._getConfig().path.themes + `/**/${CONSTANTS.YAML.THEME}`,
      { nodir: true }
    );
    ...
    each(this.config.path, (val, key) => {
      if (key !== CONSTANTS.KEY.SOURCE) {
        ...
        let rootPath = key === CONSTANTS.KEY.DESTINATION ?
          this._getConfig().path.destination : this.config.path.source;
  ...
  async read() {
    let pathDestination = this._getConfig().path.destination;
