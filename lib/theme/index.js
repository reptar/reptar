...
export default class Theme {
  ...
  _getThemesFromPackageJson(directory = '') {
    ...
    const nodeModulesPath = path.resolve(config.path.source, 'node_modules');
  ...
  update() {
    ...
    const fileSystemYamls = glob.sync(
      config.path.themes + `/**/${CONSTANTS.YAML.THEME}`,
      { nodir: true }
    );

    const moduleYamls = this._getThemesFromPackageJson(config.path.source);
