...
const CollectionPagination = require('./collection-pagination');
      ...
      this.pages.forEach((collectionPage, index) => {
        ...
        if (next) {
          Object.assign(collectionPage.metadata, {
            ...
            next: next.metadata.page,

            // The URL of next page. '' if the current page is the last.
            next_link: next.destinationRelative
          });
        }
      });
    }
  ...
  write() {
    ...
    if (this.files) {
      map(this.files, file => {
        ...
        // fs.outputFileSync(file.destination, file.render(this.layout), 'utf8');
      });
    }
    ...
    if (this.pages.length) {
      this.pages.forEach(collectionPage => {
        ...
        //   collectionPage.render(this.pagination.layout),
        //   'utf8'
        // );
      });
    }
